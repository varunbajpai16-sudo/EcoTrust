...
export const processFactoryReading = async (cemsReading, factory) => {
  ...
  // Extract factory details safely
  const factoryId =
    factory.factoryId ||
    factory._id?.toString() ||
    cemsReading.factoryId?.factoryId ||
    cemsReading.factoryId?._id?.toString() ||
    "FAC-DEFAULT";

  const factoryName =
    factory.name ||
    cemsReading.factoryId?.name ||
    `Plant ${factoryId.slice(-4)}`;

  const sensorId =
    cemsReading.sensorId?.sensorId ||
    cemsReading.sensorId?._id?.toString() ||
    "SENSOR-01";

  // 1. Fetch recent history & baseline
  const pastReadings = await getFactoryHistoricalReadings(factoryId);
  let baseline = await SensorBaseline.findOne({ sensorId });

  // 2. Run Layer 1 (Rule-based Validation)
  const l1 = runLayer1Validation(cemsReading, pastReadings);

  // 3. Run Layer 2 (Cross-Correlation with Power Load)
  const l2 = runLayer2Agents(cemsReading, baseline, factory);
  ...
  const validationRecord = await ValidationResult.create({
    ...
    rawReading: {
      ...
      pressure: cemsReading.pressure,
      flowRate: cemsReading.flowRate,
      electricityConsumption: factory.electricityConsumption || 0,
    },
    trustScore,
    verdict,
    layer1Checks: l1.checks,
  ...
  // 8. Create Alert if Suspicious / Tampered
  if (verdict === "TAMPERED" || verdict === "SUSPICIOUS" || trustScore < 70) {
    const severity = verdict === "TAMPERED" || trustScore < 50 ? "CRITICAL" : "HIGH";
    await Alert.create({
      factoryId,
      factoryName,
      sensorId,
